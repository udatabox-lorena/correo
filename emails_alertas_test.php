...
mysql_select_db('boxroot_test_juridico') or die('No se pudo seleccionar la base de datos');
/* $link = mysql_connect('localhost', 'boxroot_ju_test', 'Doctor2015A') or die('No se pudo conectar: ' . mysql_error());
...
//echo "Consulta: " . $query_conf . "<br><br>";
        ...
        while ($fila = mysql_fetch_array($result_conf, MYSQL_ASSOC)) {
            ...
            if (dias_restantes(calcular_dias($fila['id'])) > 0) {
                ...
                $mensaje = "Recuerda que debes realizar la fase del expediente <b>" . $fila['numero']
                        ...
                        . ".</br></br> Te quedan aproximadamente " . dias_restantes(calcular_dias($fila['id'])) . " dias.";
                $band = true;
            }
...
//BUSCAR ABOGADO
            if (!is_null($fila['abogado_id'])) {
                ...
                if ($band) {
                    $ultimo_correo = strftime("%Y-%m-%d %H:00:00");
                    $query_update = "";
